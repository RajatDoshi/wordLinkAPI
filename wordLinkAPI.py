...
def returnWordInfo(word):
	...
	for i in syn:
		info.append(b)
		info[j]["Key"] = i.name()
		info[j]["SynsetID"] = i.offset()
		info[j]["SynsetGloss"] = i.definition()
		j = j + 1
	return info
...
Input: The Common Name of a Part (dictionary key is "part")
Output: Returns Information of the part as a dictionary inside 1 list
"""
...
def returnInfo():
	data = request.get_json()
	word = data["part"][0]
	return jsonify(returnWordInfo(word))
""" 
Purpose: Adress For Identifying Correct Synset for a Part Annotation Linkage Based on Similarity Scores
Input: Current Part and List of Other Parts in an Object (keys are "otherPart" and "currentPart")
